...
namespace ChandlerHome.Helpers
{
    ...
    {
        public static IServiceCollection AddChandlerHomeServices(this IServiceCollection serviceCollection)
        => serviceCollection
        ...
        public static void SwitchTo(this SwitchEntity? switchToFlip, string? newState)
        {
            if (switchToFlip?.State != newState)
            {
                if (newState == "on")
                {
                    switchToFlip?.TurnOn();
        ...
        public static bool IsDoorOpen(BinarySensorEntity entity)
        {
            return entity.State.Equals("on", StringComparison.OrdinalIgnoreCase) ? true : false;
        }
    }
}
