namespace ChandlerHome.apps.HassModel.Bedroom.Lighting
{
    ...
    internal class ClosetLight : Bedroom
    {
        ...
        public ClosetLight(IHaContext ha) : base(ha)
        {
            ...
            _entities.BinarySensor.ClosetMotionMotionDetection.StateChanges().Where(e => e.New.IsOn())
                .Subscribe(x =>
                {
                    _entities.Light.BedroomClosetLight.TurnOn(brightnessPct: 100, rgbwwColor: new int[] { 0, 0, 0, 255, 0 });
                });

            _entities.BinarySensor.ClosetMotionMotionDetection.StateChanges().Where(e => e.New.IsOff())
                .Subscribe(x =>
                {
            ...
            .Subscribe(_ =>
           {
               ...
               else if (offTime == null && _entities.BinarySensor.ClosetMotionMotionDetection.IsOff() && _entities.Light.BedroomClosetLight.IsOn())
               {
                   offTime = DateTime.Now;
               }
           });
