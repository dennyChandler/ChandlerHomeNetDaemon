...
namespace ChandlerHome.apps.HassModel.Bedroom.Switches
{
    ...
    internal class NightlightSwitch : Bedroom
    {
        ...
        public NightlightSwitch(IHaContext ha) : base(ha)
        {
            ...
            _entities.BinarySensor.BedroomMotion.StateChanges().Where(e => e.New.IsOn() && DateTime.Now.Hour > 0 && DateTime.Now.Hour < 4)
                .Subscribe(x =>
                {
                    if (!_entities.Sensor.RocinanteSessionstate.State.Equals("InUse", StringComparison.OrdinalIgnoreCase))
                    {
                        if (!_entities.BinarySensor.DennysPhoneIsCharging.IsOn())
                        {
                            _entities.Switch.BedroomNightlight.TurnOn();
                            onTime = DateTime.Now;
                        }
